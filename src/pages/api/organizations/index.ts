...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  async function createOrganization() {
    ...
    if (body?.erp_integration?.length > 0) {
      const create_erp_integration = body.erp_integration;
      body.erp_integration = {
        create: create_erp_integration,
      };
    } else {
      delete body.erp_integration;
    }
