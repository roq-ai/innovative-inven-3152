...
function InventoryListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<InventoryInterface[]>(
    ...
    () =>
      getInventories({
        relations: ['organization', 'financial_detail.count'],
      }),
  );
  ...
  const handleView = (id: string) => {
    if (hasAccess('inventory', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/inventories/view/${id}`);
    }
  };
      ...
      <Box bg="white" p={4} rounded="md" shadow="md">
        ...
        <Flex justifyContent="space-between" mb={4}>
          ...
          {hasAccess('inventory', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/inventories/create`} passHref legacyBehavior>
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('financial_detail', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>{record?._count?.financial_detail}</Td>
                    )}
                    <Td>
